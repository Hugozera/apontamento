...
import { formatarMinutosParaHora, formatarHora } from "../../../utils/dateUtils";
      ...
      <html>
        ...
        <body>
          ...
          <table>
            ...
            .map(
                (r) => {
                    // Função auxiliar para formatar a hora com fallback
                    const formatarHoraComFallback = (valor, campo) => {
                        try {
                            // Tenta formatar normalmente primeiro
                            const formatado = formatarHora(valor);
                            if (formatado && formatado !== '-') return formatado;

                            // Se não funcionar, verifica se é um objeto Timestamp do Firebase
